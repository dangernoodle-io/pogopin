...
- `internal/cli/` — cobra root + CLI subcommands (decode, server, statusline)
...
- Status model: each pogo server process writes its OWN file, `~/.cache/pogopin/status/<pid>.json` (not a single shared `status.json`) — avoids a portless session's 15s heartbeat clobbering a concurrent session's port entry (last-writer-wins on one shared file). `internal/status.ReadLivePorts(sessionID, mode)` (Go, BR-76) is the reader: globs the status dir, drops a file's ports when its owning process is dead or `updated_at` is older than the mode's staleness window (45s default / 30s fresh-only, 3x/2x the heartbeat, guards PID reuse), and filters to the caller's session — an empty `sessionID` renders nothing rather than falling back to unfiltered (the cross-session port leak fix). Honors a `POGOPIN_STATUS_DIR` override (checked live on every call, not just at process start). Fully fail-open — bad/missing files contribute nothing, never throws. `plugin/scripts/pre-tool-port-check.js` still carries its own small inlined status-dir reader (formerly the shared `status-lib.js`, retired alongside `statusline.js`) since it's the only remaining JS consumer.
- `plugin/scripts/pre-tool-port-check.js` — warn-only PreToolUse hook (BR-31): reads the merged live-ports view via its inlined reader, warns via `additionalContext`/`systemMessage` (never denies) only when the target port is `running` under a *different* `CLAUDE_CODE_SESSION_ID` than the calling hook's and that session's owning process is still alive. Silent on the normal same-session flow, and gracefully degrades to no-warning on older servers or when `CLAUDE_CODE_SESSION_ID` is unset (status entries lack `session_id`).
- `pogo statusline` (Go CLI, `internal/cli/statusline.go`, BR-76) — replaces the retired `plugin/scripts/statusline.js` + `status-lib.js`. Reads the Claude Code statusline stdin contract (`session_id`, `cwd`), resolves session identity (`POGOPIN_SESSION_ID` > stdin `session_id` > `CLAUDE_CODE_SESSION_ID` > `""`), and renders serial port state via `status.ReadLivePorts`. Visibility mode is configurable via `POGOPIN_STATUSLINE_MODE`: `always` (default) prints `serial: idle` when no ports; `ports-only` exits silently when none; `fresh-only` renders only ports updated within the last 30s, exiting silently when none qualify. Unknown/empty mode values fall back to `always`. Fully fail-open — bad stdin, an unresolved session, or a read error all render nothing (or idle text) rather than an error; the command itself always returns nil to cobra.
