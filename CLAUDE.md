...
- `plugin/scripts/statusline.js` — Node.js widget for ccstatusline custom-command that renders serial port state from the merged live-ports view; when `CLAUDE_CODE_SESSION_ID` is set, filters to that session's own ports only
