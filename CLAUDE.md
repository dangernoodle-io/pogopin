...
- `pogo statusline` (Go CLI, `internal/cli/statusline.go`, BR-76/BR-92) — replaces the retired `plugin/scripts/statusline.js` + `status-lib.js`. Reads the Claude Code statusline stdin contract and renders a single `pogopin: ` label with THREE ownership states, when the BR-92 indicator is on (default; `POGOPIN_STATUSLINE_FOREIGN` off/0/false disables it — falls back to own-only via `status.ReadLivePorts`, interim knob superseded by the BR-95 config epic): own ports (this session's sessionID resolved and matches — full detail `<base(port)>@<baud> <mode> <lines>L`), foreign ports (sessionID resolved but a port belongs to a different known session — collapsed `⚠ <base(port)>` in red), and unknown ports (this session's sessionID never resolved, so ownership of ANY port can't be proven — every port with a real session id collapses to `? <base(port)>` in yellow instead of being red-flagged or dropped). Collapsed groups (foreign or unknown — mutually exclusive per invocation) render first, then own groups, joined by ` | `. A port with no session id, or an empty port path, is skipped entirely. Session-id resolution and `POGOPIN_STATUSLINE_MODE` visibility semantics: see the wiki CLI page (`pogo statusline` section). Fully fail-open — bad stdin or a read error render nothing (or idle text) rather than an error; the command itself always returns nil to cobra.
