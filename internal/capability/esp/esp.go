...
func (c Capability) Attach(r *shesha.Registrar) error {
	...
	shesha.AddTool(r, &mcpx.Tool{
		Name:        "esp_info",
		Description: "Get ESP device info. Returns chip info by default (chip_name, manufacturer_id, device_id, flash_size, mac, chip_revision, features). Pass include=security for secure boot/flash encryption status, or include=chip,security for both.",
