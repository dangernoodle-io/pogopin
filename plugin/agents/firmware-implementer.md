...
Use `serial_list` to confirm the port, `esp_flash` or `flash_external` to flash, `serial_start`/`serial_read` to observe boot output, `decode_backtrace` on any panic frame.
